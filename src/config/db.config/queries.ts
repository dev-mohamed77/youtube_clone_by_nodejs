export default {
  ...
  ADD_VIDEOS:
    "INSERT INTO videos (videos_user_id, videos_title, videos_description, videos_url, videos_image_url, duration, videos_created_at) VALUES ($1, $2, $3, $4, $5, $6, $7) returning *",
  GET_VIDEOS:
  ...
  GET_SUBSCRIPTION:
    "SELECT * FROM subscriber WHERE subscriber_user_to=$1 AND subscriber_user_from=$2",
  GET_SUBSCRIPTION_COUNT:
    "SELECT count(*) FROM subscriber WHERE subscriber_user_from=$1",
  GET_SUBSCRIPTION_WITH_ID_LIMIT:
    "SELECT s.subscriber_id, s.subscriber_user_to, json_build_object('user_id',u.users_id,'user_fullname',u.users_fullname,'users_username',u.users_username,'users_email',u.users_email,'users_image_url',u.users_image_url) AS subscriber_user_from, s.subscriber_created_at FROM subscriber s JOIN users u ON s.subscriber_user_from = u.users_id WHERE subscriber_user_to=$1 LIMIT $2 OFFSET ($3 - 1) * $2",
  GET_SUBSCRIPTION_WITH_ID_ALL:
    "SELECT s.subscriber_id, s.subscriber_user_to, json_build_object('user_id',u.users_id,'user_fullname',u.users_fullname,'users_username',u.users_username,'users_email',u.users_email,'users_image_url',u.users_image_url) AS subscriber_user_from, s.subscriber_created_at FROM subscriber s JOIN users u ON s.subscriber_user_from = u.users_id WHERE subscriber_user_to=$1",
};
